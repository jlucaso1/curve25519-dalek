...
> **This figure was published wrong twice, both times too small.** The first
> kernel consumed its result with `compress` — a field inversion, most of what
> this change removes — giving 103 616 → 71 191, −31.29%. The second replaced
> that with an Edwards addition, giving 72 650 → 40 225, −44.63%. Every
> prepared point in this kernel is valid, so the `Some` arm runs on every
> iteration and *any* work inside it lands in both arms of the comparison,
> where a constant added to both pulls the ratio toward 1. Both versions
> therefore understated the change, for the same reason, in the same direction.
>
> The kernel now makes the result observable with a reference barrier —
> `black_box(&q)`, no group operation — which is what `edwards_table_create`
> uses and what this document should have reached for the first time. The
> lesson is not "avoid `compress`"; it is that a kernel measuring a cheap
> operation must consume its result with something cheaper than the operation.
...
| **N — `to_edwards`'s second exponentiation** | **Changed.** `MontgomeryPoint::to_edwards` formed the Edwards `y` with a full Fermat inversion and then handed it to `decompress`, whose `sqrt_ratio_i` runs the same chain again. Decompression needs only the ratio, and `EdwardsPoint` is projective, so `yn`/`yd` become `Y`/`Z` and two squarings plus three multiplications replace the inversion: **70 492 → 38 033, −46.05%** on the XEdDSA verification entry point. Congruent, not limb-identical; checked against a verbatim copy of the old code over 256 inputs and both signs (§13.14). |
